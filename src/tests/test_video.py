...
class VideoRequests(unittest.TestCase):
    ...
    def test_video_upload_valid_extension(self):
        ...
        for test in test_files:
            ...
            self.mock_upload.send_video.return_value = upload_res
            # print(open(test, 'rb'))
            video = open(test, 'rb')
            res = self.client().post('/api/video',data= dict(
            ...
            self.mock_upload.send_video.assert_called_with(mock.ANY,'Test video '+ test,'ipsy')
    ...
    def test_video_propagate_exceptions(self):
        ...
        self.mock_upload.send_video.side_effect = Exception
